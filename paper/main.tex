...
\algrenewcommand\alglinenumber[1]{\footnotesize\arabic{alglinenumber}}
...
%% Authors
\author[inst1]{Yan Wang}

\author[inst2,cor1]{Dapeng Lang}
\ead{langdapeng@outlook.com}
...
\affiliation[inst2]{organization={Harbin Engineering University},
            city={Harbin},
            postcode={150001},
            state={Heilongjiang},
            country={China}}
...
%% Keywords
\begin{keywords}
large language models \sep jailbreak attack \sep active subspace \sep Fisher information matrix \sep gradient optimization \sep safety alignment
\end{keywords}
...
\For{$t = 1, 2, \ldots, T$}
    ...
    \EndIf
    \State Update logits (STE): $\mathbf{l}_i \leftarrow \mathbf{l}_i - \eta \cdot \nabla_{\mathbf{l}_i} \tilde{L}$ for $i = 1, \ldots, m$
    \State Anneal temperature: $\tau_{t+1} \leftarrow \max(\tau_{\min}, \gamma \tau_t)$
\EndFor
\State \Return Top-1 tokens: $\mathbf{S}^* = [\arg\max \mathbf{l}_1; \ldots; \arg\max \mathbf{l}_m]$
\end{algorithmic}
\end{algorithm}
...
The PPL of ASA (15.3) is comparable to natural text (GPT-2 PPL $\approx$ 8--12 on clean English), while GCG produces suffixes with PPL = 4250.6 that are trivially detected. I-GCG reduces PPL to 1250.3 via multi-target templates but still fails the PPL $< 100$ filter 27.9\% of the time. ASA's 97.2\% filter pass rate confirms that subspace-projected optimization naturally constrains token selection to semantically coherent regions of the vocabulary.
...
\paragraph{Core Component Analysis.} Removing active subspace projection (i.e., using full-space gradients) degrades ASR by 6.8\% and increases iterations by 206\%, confirming that subspace-constrained optimization is the single most important contributor to both effectiveness and efficiency. Replacing incremental SVD with full batch eigendecomposition increases memory from 5.4 MB to 52.1 MB (865\% increase), validating Theorem~\ref{thm:subspace_error} that the incremental procedure is both accurate and memory-efficient.

\paragraph{Continuous-to-Discrete Mapping.} Removing Gumbel-Softmax (replacing with top-$k$ sampling) degrades ASR by 4.9\% and increases PPL from 15.3 to 18.6, confirming that the differentiable relaxation is essential for effective gradient flow. Removing STE (using only soft gradients in the forward pass) increases PPL to 2960.5, demonstrating that the hard forward pass is critical for generating discrete tokens that the LLM can actually process.
...
ASA demonstrates significantly higher defense evasion capability than all baselines. Against perplexity-based filtering (threshold PPL $< 100$), ASA achieves 97.2\% pass rate while GCG and Soft-GCG are completely blocked. Against SmoothLLM perturbation defense, ASA retains 56.2\% ASR compared to 28.4\% for GCG (+97.9\% relative improvement). Against SafeDecoding, ASA achieves 61.4\% ASR compared to 52.3\% for I-GCG. The natural coherence of ASA-generated suffixes (PPL = 15.3) is the key enabler: perplexity-based defenses are inherently less effective against attacks that produce human-readable text.
